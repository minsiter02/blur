...
class TaskFrame(Frame):
    ...
    def on_press(self, event): #마우스를 처음 누룰 때 시작 좌표 설정
        ...
        self.rect.append( self.task_canvas.create_rectangle(self.start_x, self.start_y, self.start_x, self.start_y, fill=rgb_to_hex(rgb) ,outline="blue", width=2))

    def on_drag(self, event): # 드래그 중일 때 
        self.task_canvas.coords(self.rect[-1], self.start_x, self.start_y, event.x, event.y)
    ...
    def delete(self,event):
        self.task_canvas.delete(self.rect[-1])
        self.rect.pop()
