...
class TaskFrame(Frame):
    def __init__(self, parent, main):
        ...
        self.task_canvas_width = None
        self.task_canvas_height = None
        self.start_x = None
        self.start_y = None
        self.end_x = None
        self.end_y = None
    ...
    def undo_shape(self): # 임시 undo 함수.
        cur_id = self.shape_hist_id
        history = self.shape_history
        if cur_id == 0: return  #초기 이미지일때 지우기 안됨.
         # 파일구조가 조금 복잡해서 (시간순으로 정렬하려다 보니) id로 해당 index를 찾고, 그 전 index를 구하는 방식으로 구현
    ...
    def redo_shape(self):
        cur_id = self.shape_hist_id
        history = self.shape_history
        if cur_id == history[-1][0]: return None
         # 파일구조가 조금 복잡해서 (시간순으로 정렬하려다 보니) id로 해당 index를 찾고, 그 전 index를 구하는 방식으로 구현
        cur_id_pos = find_element_idx(cur_id,history)  # 현재 id의 위치
        target_id_pos = cur_id_pos + 1  # 1칸 왼쪽의 위치
